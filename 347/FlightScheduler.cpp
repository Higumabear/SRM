...
#define LLINF 1LL << 60
#define EPS 1e-6
#define eul 2.71828182845904523536
...
public:
  ...
  double minimizeFuel(int distance, int K, int emptyMass, int takeoffFuel){
    ...
    while(ub - lb > 100){
      int mid = (ub + lb) / 2;
      int T = (lb + mid) / 2, U = (mid + ub) / 2;
      double fuel1 = 1.0 * T * emptyMass * exp(distance * 1.0 / T / K) 
	- 1.0 * T * emptyMass + 1.0 * T * takeoffFuel;
      double fuel2 = 1.0 * U * emptyMass * exp(distance * 1.0 / U / K) 
	- 1.0 * U * emptyMass + 1.0 * U * takeoffFuel;
      if(fuel1 > fuel2) lb = T;
      else ub = U;
    }
    for(int i = lb; i <= ub; i++){
      double fuel = 1.0 * i * emptyMass * exp(distance * 1.0 / i / K) 
	- 1.0 * i * emptyMass + 1.0 * i * takeoffFuel;
      ans = min(ans, fuel);
    }
    return ans;
  }
