...
public:
  long long countDesigns(int numCheckpoints, vector <int> checkpoint1, vector <int> checkpoint2){
    ...
    vector<int> num(numCheckpoints, 0);
    vector<vector<bool> > co(numCheckpoints, 
			     vector<bool>(numCheckpoints, false));
    for(int i = 0; i < N; i++) {
      num[checkpoint1[i] - 1]++, num[checkpoint2[i] - 1]++;
      co[checkpoint1[i] - 1][checkpoint2[i] - 1] = 
	co[checkpoint2[i] - 1][checkpoint1[i] - 1] = true;
    }
    ...
    for(int i = 0; i < numCheckpoints; i++)
      for(int j = i; j < numCheckpoints; j++)
	for(int k = j; k < numCheckpoints; k++)
	  if(co[i][j] && co[j][k] && co[k][i]) ans++;
    return ans;
  }
