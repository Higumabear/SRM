...
public:
  int getNumber(vector <int> guesses, vector <int> answers){
    ...
    for(int i = 0; i < N; i++) 
      p[guesses[i] - answers[i]]++, p[guesses[i] + answers[i]]++;
    
    vector<int> ans;
    map<int, int>::iterator it = p.begin();
    for(; it != p.end(); it++)
      if(it->second == N && 1 <= it->first && it->first <= 1000000000) 
	ans.push_back(it->first);
    
    if(N == 1 || p.size() == 2){
      bool ub = guesses[0] + answers[0] <= 1000000000;
      bool lb = guesses[0] - answers[0] >= 1;
      if(ub && lb) return -1;
      if(ub) return guesses[0] + answers[0];
      if(lb) return guesses[0] - answers[0];
      if(!ub && !lb) return -2;
    }


    return ans.size() != 1 ? -2 : ans[0];
  }
