...
public:
  double getAverage(vector <string> ratings){
    vector<int> rate;
    string ra = accumulate(ALL(ratings), string(" "));
    stringstream ss(ra);
    int t; while(ss >> t) rate.push_back(t);
    
    int elly = rate[0];
    sort(ALL(rate), greater<int>());
    int R = ((rate.size() + 19) / 20);
    ...
    for(int i = 0; i < N;  i += R){
      vector<int> g;
      for(int j = i; j < N && j - i < R; j++) g.push_back(rate[j]);
      if(find(ALL(g), elly) != g.end()){
	sum += elly;
	num++;
      }else if((int)g.size() < R){
	double ans = 0.0;
	for(int j = 0; j < R; j++){
	  if(j < (int)g.size()) ans += (sum + g[j]) / (num + 1);
	  else ans += sum / num;
	}
	return ans / R;
      }else{
	sum += accumulate(ALL(g), 0) / (double)R;
	num++;
      }
    }
    return sum / num;
